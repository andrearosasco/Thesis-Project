...
def run(config):
    ...
    if log_config['wandb']:
        wandb.init(project="scifar10-distill", name=log_config['wandb_name'])
    ...
    for task_id, task in enumerate(run_config['tasks'], 0):
        ...
        steps = int(len(bufferloader) * param_config['epochs'])

        for step in range(steps):

            buffer_loss, buffer_accuracy = train(net)

            if int(round(steps * 0.05)) <= 0 or step % int(round(steps * 0.05)) == int(round(steps * 0.05)) - 1 or step == 0:
        ...
        for b in range(param_config['buffer_size']):
            ...
            if log_config['wandb'] and b == 0:
                mean, std = data_config['test_transform'].transforms[-1].mean, data_config['test_transform'].transforms[-1].std
                x, y = next(iter(DataLoader(buff, batch_size=param_config['buffer_size']*2, shuffle=False)))
